...
namespace elFinder.NetCore.Drivers.FileSystem
{
    ...
    public class FileSystemDriver : BaseDriver, IDriver
    {
        ...
        public async Task<JsonResult> ExtractAsync(FullPath fullPath, bool newFolder)
        {
            ...
            if (newFolder)
            {
                rootPath = PathHelper.SafelyCombine(rootPath, Path.GetFileNameWithoutExtension(fullPath.File.Name));
            ...
            using (var archive = ZipFile.OpenRead(fullPath.File.FullName))
            {
                ...
                foreach (ZipArchiveEntry entry in archive.Entries)
                {
                    try
                    {
                        ...
                        string file = PathHelper.SafelyCombine(rootPath, entry.FullName)
        ...
        public async Task<JsonResult> MakeDirAsync(FullPath path, string name, IEnumerable<string> dirs)
        {
            ...
            {
                var newDir = new FileSystemDirectory(PathHelper.SafelyCombine(path.Directory.FullName, name));
            ...
            foreach (string dir in dirs)
            {
                ...
                var newDir = new FileSystemDirectory(PathHelper.SafelyCombine(path.Directory.FullName, dirName));
                await newDir.CreateAsync();
        ...
        {
            var newFile = new FileSystemFile(PathHelper.SafelyCombine(path.Directory.FullName, name));
        ...
        public async Task<JsonResult> PasteAsync(FullPath dest, IEnumerable<FullPath> paths, bool isCut, IEnumerable<string> renames, string suffix)
        {
            ...
            foreach (var src in paths)
            {
                if (src.IsDirectory)
                {
                    var newDir = new FileSystemDirectory(PathHelper.SafelyCombine(dest.Directory.FullName, src.Directory.Name));
                ...
                {
                    var newFile = new FileSystemFile(PathHelper.SafelyCombine(dest.Directory.FullName, src.File.Name));
        ...
        public async Task<JsonResult> RenameAsync(FullPath path, string name)
        {
            ...
            if (path.IsDirectory)
            {
                var newPath = new FileSystemDirectory(PathHelper.SafelyCombine(path.Directory.Parent.FullName, name));
            ...
            {
                var newPath = new FileSystemFile(PathHelper.SafelyCombine(path.File.DirectoryName, name));
        ...
        public async Task<JsonResult> UploadAsync(FullPath path, IEnumerable<IFormFile> files, bool? overwrite, IEnumerable<FullPath> uploadPaths, IEnumerable<string> renames, string suffix)
        {
            ...
            {
                var fileInfo = new FileInfo(PathHelper.SafelyCombine(path.Directory.FullName, rename));
                string destination = PathHelper.SafelyCombine(path.Directory.FullName, $"{Path.GetFileNameWithoutExtension(rename)}{suffix}{Path.GetExtension(rename)}");
            ...
            foreach (var file in files)
            {
                ...
                var fileInfo = new FileInfo(PathHelper.SafelyCombine(destination, Path.GetFileName(file.FileName)));
                ...
                if (fileInfo.Exists)
                {
                    ...
                    else
                    {
                        ...
                        using (var fileStream = new FileStream(PathHelper.SafelyCombine(fileInfo.DirectoryName, newName), FileMode.Create))
                        {
                            await file.CopyToAsync(fileStream);
                        }
                        response.Added.Add(await BaseModel.CreateAsync(new FileSystemFile(newName), path.RootVolume));
        ...
        public async Task<FileStreamResult> ZipDownloadAsync(FullPath cwdPath, string archivedFileKey, string downloadFileName, string mimeType)
        {
            ...
            var tempFile = new FileInfo(PathHelper.SafelyCombine(tempPath, archivedFileKey));
        ...
        private void DirectoryCopy(string sourceDirName, string destDirName, bool copySubDirs)
        {
            ...
            foreach (FileInfo file in files)
            {
                ...
                string temppath = PathHelper.SafelyCombine(destDirName, file.Name);
            ...
            if (copySubDirs)
            {
                foreach (DirectoryInfo subdir in dirs)
                {
                    ...
                    string temppath = PathHelper.SafelyCombine(destDirName, subdir.Name);
